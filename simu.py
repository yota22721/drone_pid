...
class Controller:
    def __init__(self):
        Kp_pos = [4,4, 0.08] # proportional [x,y,z]
        Ki_pos = [0.004, 0.004, 0.0003]  # integral [x,y,z]
        Kd_pos = [19, 17, 0.35] # derivative [x,y,z]

        # Gains for 
        # angle controller
        Kp_ang= [4, 4, 11] # proportional [x,y,z]
        Ki_ang = [0.03, 0.03, 0.1]  # integral [x,y,z]
        Kd_ang = [4, 6, 6] # derivative [x,y,z]
    ...
    def controller(self,u,x,k,dt):
            ...
            if x[11, k] < 0.5 and np.abs(x[9,k]) <0.02 and np.abs(x[10,k]) <0.02:
            ...
            for i in range(4):
                 motor_speeds[i] = np.clip(motor_speeds[i]*kt, 0, 3.25)
                 motor_speeds[i] /=kt
